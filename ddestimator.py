...
class ddestimator:
    ...
    CAMERA_CALIBRATION_MATRIX = np.float32([[1253.0839, 0., 619.5],
                                            [0., 1253.0839, 439.5],
                                            [0., 0., 1.]])
    ...
    def euler_decomposition(self, projmat):
        sin_x = math.sqrt(
            projmat[0, 0] * projmat[0, 0] + projmat[1, 0] * projmat[1, 0])
    ...
    def get_med_eulers(self, ts_threshold=2000):
        ...
        count = self.log[(self.log.ts > ts) & (
            self.log.key == 'euler_c')]['value'].count()
        if count > round(ts_threshold/200):
            med_x = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_x')]['value'].median()
            med_y = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_y')]['value'].median()
            med_z = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_z')]['value'].median()
            med_c = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_c')]['value'].median()
    ...
    def est_head_dir_over_time(self, ts_threshold=1000, angle_threshold=45):
        ...
        count = self.log[(self.log.ts > ts) & (
            self.log.key == 'euler_c')]['value'].count()
        if count > round(ts_threshold/200):
            min_x = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_x')]['value'].apply(abs).min()
            min_y = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_y')]['value'].apply(abs).min()
            min_z = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_z')]['value'].apply(abs).min()
            min_c = self.log[(self.log.ts < ts) & (
                self.log.key == 'euler_c')]['value'].min()
    ...
    def draw_bounding_cube(self, frame, bc_2d_coords, color, euler):
        ...
        for from_pt, to_pt in np.array([[0, 1], [1, 2], [2, 3], [3, 0],
                                        [4, 5], [5, 6], [6, 7], [7, 4],
                                        [0, 4], [1, 5], [2, 6], [3, 7]]):
            cv2.line(frame, tuple(bc_2d_coords[from_pt]), tuple(
                bc_2d_coords[to_pt]), color)
    ...
    def est_gaze_dir_over_time(self, ts_threshold=2000, angle_threshold=27.5):
        ts = self.get_current_ts() - ts_threshold
        count = self.log[(self.log.ts > ts) & (
            self.log.key == 'gaze_D')]['value'].count()
        if count > round(ts_threshold/200):
            avg_l = self.log[(self.log.ts < ts) & (
                self.log.key == 'gaze_L')]['value'].mean()
            avg_r = self.log[(self.log.ts < ts) & (
                self.log.key == 'gaze_R')]['value'].mean()
            med_d = self.log[(self.log.ts < ts) & (
                self.log.key == 'gaze_D')]['value'].median()
    ...
    def draw_gaze_line(self, frame, gl_2d_coords, color, gaze_D):
        ...
        for from_pt, to_pt in np.array([[0, 1]]):
            cv2.line(frame, tuple(gl_2d_coords[from_pt]), tuple(
                gl_2d_coords[to_pt]), color)
    ...
    def calc_kss(self, ts_threshold=10000):
        ts = self.get_current_ts() - ts_threshold
        count = self.log[(self.log.ts > ts) & ((self.log.key == 'distracted') | (
            self.log.key == 'drowsiness'))]['value'].count()
        if count > round(ts_threshold/200):
            sum_distracted = self.log[(self.log.ts > ts) & (
                self.log.key == 'distracted')]['value'].sum()
            sum_drowsy = self.log[(self.log.ts > ts) & (
                self.log.key == 'drowsiness')]['value'].sum()
    ...
    def create_progress_bar(self, width, height, percentage=0, status=None):
        ...
        if size > 0:
            cv2.rectangle(image, (9, 9), (9 + size, height - 9),
                          (0, 255, 0), cv2.FILLED)
        if status is not None:
            cv2.putText(image, status, (15, height - 13),
                        cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), thickness=3)
            cv2.putText(image, status, (15, height - 13),
                        cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), thickness=1)
        return image

    def draw_progress_bar(self, frame, width, height, percentage=0, status=""):
        progressbar = self.create_progress_bar(
            width, height, percentage, status)
        y_offset = frame.shape[0] - height
        x_offset = 0
        frame[y_offset:y_offset + progressbar.shape[0],
              x_offset:x_offset + progressbar.shape[1]] = progressbar
        return frame
    ...
    def push_to_log(self, key, value):
        ...
        self.log = self.log.append(
            {'ts': ts, 'key': key, 'value': value}, ignore_index=True)
    ...
    def purge_from_log(self, ts_threshold, key):
        if self.purge:
            ...
            self.log = self.log.drop(
                self.log[(self.log.ts < ts) & (self.log.key == key)].index)
        return self.log['ts'].count()
            ...
            if max_val > mar_threshold:
                ...
                if max_val_ts > q1_ts and max_val_ts < q3_ts:
                    ...
                    if mse_model > 0.2 and mse_yhat < 0.1:
                        self.push_to_log('drowsiness', math.ceil(
                            self.weights[3]*(mse_model*2)))
